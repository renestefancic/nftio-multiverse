  ...
  return (
    <section className="py-24 px-4 md:px-8 border-t border-white/5 relative overflow-hidden bg-brand-black">
      ...
      <div className="absolute top-0 left-1/2 -translate-x-1/2 w-full h-full max-w-7xl opacity-30 pointer-events-none">
        <div className="absolute top-[20%] left-[10%] w-96 h-96 bg-brand-primary/10 blur-[100px] rounded-full"></div>
      </div>
      ...
      <div className="max-w-7xl mx-auto relative z-10">
        ...
        <div className="relative">
          ...
          <div className="hidden md:block absolute top-12 left-0 w-full h-0.5 bg-brand-surface/50 -z-10">
            <div className="h-full bg-brand-primary/50 w-[25%] shadow-[0_0_15px_#7835E5]"></div>
          </div>
              ...
              return (
                ...
                >
                  ...
                  <div className={`hidden md:flex absolute top-0 left-1/2 -translate-x-1/2 -translate-y-1/2 w-24 h-24 items-center justify-center -z-10 opacity-0 group-hover:opacity-100 transition-opacity duration-500`}>
                     <div className={`w-full h-full rounded-full blur-xl ${isActive ? 'bg-brand-orange/20' : 'bg-brand-primary/10'}`}></div>
                  </div>
                  ...
                  >
                    ...
                    <div className="flex justify-between items-start mb-6">
                      <div className={`
                        ...
                        ${isActive 
                          ? `${season.color} bg-brand-dark border-current` 
                      ...
                      <div className="text-right">
                         <span className={`text-sm font-bold font-mono uppercase tracking-widest block mb-2 ${isActive ? 'text-brand-accent' : 'text-gray-500'}`}>
                           {season.dateRange}
                         </span>
                         {isActive && (
                           <span className="inline-flex items-center gap-1.5 px-2 py-0.5 rounded bg-brand-orange/10 border border-brand-orange/20 text-[10px] font-bold text-brand-orange animate-pulse">
                    ...
                    {isActive ? (
                      <div className="mt-auto">
                        <div className="flex justify-between text-[10px] font-mono text-brand-orange mb-1.5">
                           <span className="flex items-center gap-1"><Clock className="w-3 h-3" /> SEASON PROGRESS</span>
                           <span>35%</span>
                        </div>
                        <div className="h-1.5 w-full bg-black/40 rounded-full overflow-hidden border border-white/5">
                          <div className="h-full bg-gradient-to-r from-brand-orange to-brand-gold w-[35%] rounded-full shadow-[0_0_10px_#FF5500]"></div>
                        </div>
                      </div>
                    ) : (
